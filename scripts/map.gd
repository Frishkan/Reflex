	...
	for current_floor : Array in map_data :
		for room : Room in current_floor :
			if room.next_rooms.size() > 0 :
				_spawn_room(room)
	## spawning boss room
	var middle := floori(MapGenerator.floor_width * 0.5)
	_spawn_room(map_data[MapGenerator.floors - 1][middle])
	
	var map_width_pixels := MapGenerator.x_dist * (MapGenerator.floor_width - 1)
