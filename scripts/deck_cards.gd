...
func cards_grid_initiate(deck_type : int) : ## a bug happening here (1 card spawns to the side, it shouldn't even exist)
	for cards in Singleton.deck[deck_type].size() :
		...
		new_card.sprite_2d.texture = CardsLibrary.ICONS[Singleton.deck[deck_type][cards - 1][0]][0]
		new_card.name_label.text = CardsLibrary.ICONS[Singleton.deck[deck_type][cards - 1][0]][2]
		new_card.short_desc.text = CardsLibrary.ICONS[Singleton.deck[deck_type][cards - 1][0]][3] ## long desc
		new_card.short_desc.label_settings.font_size = 5
		new_card.scale = CardsLibrary.ICONS[Singleton.deck[deck_type][cards - 1][0]][1]
		new_card.hand_card_name = Singleton.deck[deck_type][cards - 1][0]
